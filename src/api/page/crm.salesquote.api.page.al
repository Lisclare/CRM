...
page 50626 "CRM Sales Quotes"
{
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                part(salesQuoteLines; "CRM Sales Quote Lines")
                {
                    Caption = 'Lines';
                ...
                field("quoteAccepted"; Rec."Quote Accepted")
                {
                    ...
                    trigger OnValidate()
                    begin
                        RegisterFieldSet(Rec.FieldNo("Quote Accepted"));
                    end;
                }
                ...
                field(createdByCRM; Rec."Created by CRM")
                {
                    Caption = 'Created by CRM';

                    trigger OnValidate()
                    begin
                        RegisterFieldSet(Rec.FieldNo("Created by CRM"));
                    end;
                }
                field("crmSubject"; Rec."CRM Subject")
                {
                    Caption = 'CRM Subject';

                    trigger OnValidate()
                    begin
                        RegisterFieldSet(Rec.FieldNo("CRM Subject"));
                    end;
                }
                field(workDescription; WorkDescriptionTxt)
                {
                    Caption = 'Work Description';

                    trigger OnValidate()
                    begin
                        WorkDescriptionSet := true;
                    end;
                }
    ...
    begin
        ...
        if SalesHeader.Get(SalesHeader."Document Type"::Quote, Rec."No.") then begin
            ...
            if SalesHeader."Work Description".HasValue then begin
                SalesHeader."Work Description".CreateInStream(InS, TextEncoding::UTF8);
                InS.ReadText(Tmp);
                WorkDescriptionTxt := Tmp;
            end;
        end;
    ...
    begin
        ...
        ApplyWorkDescriptionToSalesHeader();
    ...
    local procedure ApplyWorkDescriptionToSalesHeader()
    var
        SalesHeader: Record "Sales Header";
        OutS: OutStream;
    begin
        if not WorkDescriptionSet then
            exit;

        // Your buffer has the quote no. in Rec."No."
        SalesHeader.Get(SalesHeader."Document Type"::Quote, Rec."No.");

        Clear(SalesHeader."Work Description");
        SalesHeader."Work Description".CreateOutStream(OutS, TextEncoding::UTF8);
        OutS.WriteText(WorkDescriptionTxt);
